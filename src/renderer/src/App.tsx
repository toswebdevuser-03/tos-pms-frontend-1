...
// ── Step 5a: lazy-load every modal/dashboard NOT needed for the login screen. ──
// These were previously static imports at the top of the file, which meant the
// whole app's component tree (~40 components) loaded before auth even resolved.
// CommandPalette, ProjectDetail, HomeDashboard, Topbar, ProjectSidebar, Toast,
// and FormModal stay eager above — they're needed immediately once logged in.
const MembersModal = lazy(() => import('./components/MembersModal'))
const SettingsModal = lazy(() => import('./components/SettingsModal'))
const RemindersPanel = lazy(() => import('./components/RemindersPanel'))
const AssignmentsModal = lazy(() => import('./components/AssignmentsModal'))
const OrgChartModal = lazy(() => import('./components/OrgChartModal'))
const MyTasksModal = lazy(() => import('./components/MyTasksModal'))
const MyWeekModal = lazy(() => import('./components/MyWeekModal'))
const TalentModal = lazy(() => import('./components/TalentModal'))
const BestFitModal = lazy(() => import('./components/BestFitModal'))
const AllocationModal = lazy(() => import('./components/AllocationModal'))
const TaskAllocationModal = lazy(() => import('./components/TaskAllocationModal'))
const AllocationHub = lazy(() => import('./components/AllocationHub'))
const MyAllocationModal = lazy(() => import('./components/MyAllocationModal'))
const ApprovalsModal = lazy(() => import('./components/ApprovalsModal'))
const StaffingModal = lazy(() => import('./components/StaffingModal'))
const EmployeeDashboard = lazy(() => import('./components/EmployeeDashboard'))
const ChangePasswordModal = lazy(() => import('./components/ChangePasswordModal'))
const QuoteModal = lazy(() => import('./components/QuoteModal'))
const ClientsModal = lazy(() => import('./components/ClientsModal'))
    ...
    <div className="app-shell">
      ...
      <div className="app-layout">
        ...
        <main className="main-area">
          ...
          {feature === 'talent' && (
            <Suspense fallback={null}>
              <TalentModal projects={visibleProjects} onClose={closeFeature} onToast={showToast} />
            </Suspense>
          )}
          {feature === 'bestFit' && (
            <Suspense fallback={null}>
              <BestFitModal projects={visibleProjects} onClose={closeFeature} />
            </Suspense>
          )}
          {feature === 'myAlloc' && (
            <Suspense fallback={null}>
              <MyAllocationModal projects={visibleProjects} onClose={closeFeature} onToast={showToast} />
            </Suspense>
          )}
          {feature === 'approvals' && (
            <Suspense fallback={null}>
              <ApprovalsModal onClose={closeFeature} onToast={showToast} />
            </Suspense>
          )}
          {feature === 'staffing' && (
            <Suspense fallback={null}>
              <StaffingModal projects={visibleProjects} onClose={() => { closeFeature(); queryClient.invalidateQueries({ queryKey: queryKeyFactory.projectMembers.all() }) }} onToast={showToast} />
            </Suspense>
          )}
          {(feature === 'exec' || feature === 'disc') && (
            <Suspense fallback={null}>
              <ExecDashboard key={feature} projects={visibleProjects} initialView={feature === 'disc' ? 'discipline' : 'portfolio'} onClose={closeFeature} onSelect={setSelectedId} onToast={showToast} />
            </Suspense>
          )}
          {feature === 'quote' && (
            <Suspense fallback={null}>
              <QuoteModal onClose={closeFeature} onToast={showToast} onOpenProject={async (id) => { await queryClient.invalidateQueries({ queryKey: queryKeyFactory.projects.all() }); await queryClient.invalidateQueries({ queryKey: queryKeyFactory.projectMembers.all() }); setSelectedId(id) }} />
            </Suspense>
          )}
          {(feature === 'clientData' || feature === 'clientDash') && (
            <Suspense fallback={null}>
          ...
          {feature === 'recycleBin' && (
            <Suspense fallback={null}>
              <RecycleBinModal onClose={closeFeature} onToast={showToast} onChanged={() => { queryClient.invalidateQueries({ queryKey: queryKeyFactory.projects.all() }); queryClient.invalidateQueries({ queryKey: queryKeyFactory.projectMembers.all() }) }} />
            </Suspense>
          )}
          {feature === 'org' && (
            <Suspense fallback={null}>
              <OrgChartModal onClose={closeFeature} />
            </Suspense>
          )}
          {feature === 'myTasks' && (
            <Suspense fallback={null}>
              <MyTasksModal projects={visibleProjects} onClose={closeFeature} onToast={showToast} />
            </Suspense>
          )}
          {feature === 'myWeek' && (
            <Suspense fallback={null}>
              <MyWeekModal projects={visibleProjects} onClose={closeFeature} onNavigate={goToProjectTab} />
            </Suspense>
          )}
          {feature === 'members' && (
            <Suspense fallback={null}>
              <MembersModal onClose={closeFeature} onToast={showToast} onViewDashboard={(m) => { setEmpDashMember(m); setFeature('empDash') }} />
            </Suspense>
          )}
          {feature === 'settings' && (
            <Suspense fallback={null}>
              <SettingsModal onClose={closeFeature} onToast={showToast} />
            </Suspense>
          )}
        </main>
      </div>
      ...
      {showChangePw && (
        <Suspense fallback={null}>
          <ChangePasswordModal onClose={() => setShowChangePw(false)} onToast={showToast} />
        </Suspense>
      )}
      <TaskTimer projects={visibleProjects} onToast={showToast} />
      {paletteOpen && <CommandPalette projects={visibleProjects} members={members} onClose={() => setPaletteOpen(false)} onNavigate={handlePalette} />}
      {showReminders && (
        <Suspense fallback={null}>
          <RemindersPanel projects={visibleProjects} onClose={() => { setShowReminders(false); queryClient.invalidateQueries({ queryKey: queryKeyFactory.reminders.all() }); setUnseenUpdates([]) }} onToast={showToast} onNavigate={goToProjectTab} onCleared={() => setUnseenUpdates([])} />
        </Suspense>
      )}
      {toast && <Toast key={toast.key} message={toast.message} type={toast.type} action={toast.action} onClose={() => setToast(null)} />}
    </div>
